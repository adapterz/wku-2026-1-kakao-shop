...
## 공통 페이지 부트스트랩과 보호 페이지

페이지를 먼저 표시한 뒤 테마와 로그인 상태를 확인하면 다음과 같은 화면 깜빡임이 발생할 수 있습니다.
...
이를 방지하기 위해 테마와 페이지 인증을 화면 렌더링보다 앞에서 처리합니다.

```text
개선 흐름
테마 선적용·보호 페이지 로그인 확인
→ 허용된 HTML 표시
→ 페이지별 데이터 조회와 화면 기능 실행
```

### 1. 렌더링 전 테마 부트스트랩

`public/js/bootstrap.js`는 `localStorage`에 저장된 `profile_dark_mode` 값을 읽고 `<html>` 요소에 `dark-theme` 클래스를 적용합니다.

모든 HTML에서는 이 스크립트를 CSS보다 먼저 동기 실행해야 합니다.
...
이 순서를 지키면 CSS가 첫 화면을 그리기 전에 테마가 결정되므로 페이지 이동 시 라이트모드가 잠깐 보이는 현상을 줄일 수 있습니다.

페이지별 JavaScript에서 `DOMContentLoaded` 이후 다크모드를 다시 초기화하지 않습니다. 설정 화면의 테마 변경도 `document.documentElement`의 클래스를 기준으로 처리합니다.

### 2. 보호 페이지 HTML 선검증

보호 페이지는 로그인한 사용자만 볼 수 있는 개인 화면입니다.
...
기존에는 `express.static()`이 보호 페이지 HTML을 먼저 전송한 뒤, 페이지 JavaScript가 API의 `401 login_required` 응답을 확인하고 로그인 화면으로 이동했습니다. 이 과정에서 비회원에게 보호 화면의 틀이 잠깐 보일 수 있었습니다.

`server/middleware/pageAuth.js`는 정적 파일 제공보다 먼저 실행되어 보호 HTML 요청의 세션을 확인합니다.

```text
보호 페이지 요청
→ 서버가 req.session.user 확인
→ 비회원: HTML을 보내지 않고 /login.html로 302 이동
→ 회원: 정적 HTML 제공
```

공개 페이지는 허용 목록으로 관리합니다.
...
공개 목록에 없는 신규 `.html` 페이지는 기본적으로 보호됩니다. 신규 공개 페이지를 추가할 때만 `PUBLIC_HTML_PATHS`에 경로를 명시해야 합니다.

페이지 HTML 선검증은 화면 노출을 막는 역할이고, 실제 데이터 보안은 기존 API의 `requireLogin` 미들웨어가 담당합니다. 따라서 보호 페이지 미들웨어를 추가해도 API 인증 검사는 제거하지 않습니다.

### 신규 페이지 추가 확인사항

- 모든 HTML의 `<head>`에서 `bootstrap.js`를 CSS보다 먼저 로드합니다.
- 개인 정보나 로그인 사용자 데이터가 있으면 보호 페이지로 유지합니다.
- 비회원에게 공개할 페이지일 때만 서버 공개 목록에 추가합니다.
- 사용자 데이터 API에는 기존 `requireLogin` 검사를 유지합니다.


## 선물함 세션 스토리지 캐시

선물함과 프로필은 모두 미사용·사용완료 패스를 확인하기 위해 다음 API를 호출합니다.
...
페이지를 이동할 때마다 두 API를 다시 호출하면 동일한 DB 조회가 반복되고, 사용자는 목록이 표시될 때까지 로딩 화면을 보게 됩니다. 이를 줄이기 위해 `public/js/api.js`에서 선물 목록을 `sessionStorage`에 5분 동안 캐시합니다.

### 쿠키 대신 sessionStorage를 사용하는 이유

쿠키는 로그인 세션 ID처럼 서버가 매 요청에서 확인해야 하는 작은 값에 적합합니다. 선물 목록처럼 크기가 커질 수 있는 JSON 데이터를 쿠키에 넣으면 모든 HTTP 요청에 함께 전송되어 네트워크 사용량이 늘어나고, 용량 제한과 개인 데이터 노출 위험도 커집니다.

`sessionStorage`는 현재 브라우저 탭 안에서만 데이터를 보관하고 서버 요청에는 자동으로 포함되지 않으므로 화면 표시용 임시 캐시에 적합합니다.
...
→ 5분 이내 캐시 존재: API 호출 없이 즉시 화면 표시
→ 캐시 없음 또는 만료: 선물 API 2건 호출
→ 응답을 화면에 표시하고 sessionStorage에 저장
```

캐시 키와 유효시간은 다음과 같습니다.

```text
키: iksan_gift_collections_v1
유효시간: 5분
보관 범위: 현재 브라우저 탭
```
...
캐시가 남아 있으면 변경 전 선물 상태가 표시될 수 있으므로 다음 작업이 성공하면 `clearGiftCollectionsCache()`를 호출합니다.

- 로그인 성공: 이전 계정의 캐시 제거
- 로그아웃 성공: 개인 선물 데이터 제거
- 주문 생성 성공: 새로 발급된 패스 반영 준비
- 패스 사용 완료 성공: 미사용·사용완료 탭 이동 상태 반영
- API의 `login_required` 응답: 만료된 로그인 세션의 캐시 제거

탭을 닫아도 브라우저가 `sessionStorage`를 자동으로 삭제합니다.

바코드, QR 및 개별 선물 상세는 상태와 보안 민감도가 높으므로 목록 캐시에 저장하지 않고 기존 API에서 매번 조회합니다. 캐시는 화면 응답 속도를 개선하는 기능일 뿐이며, 서버의 세션 인증과 API `requireLogin` 검사를 대체하지 않습니다.
