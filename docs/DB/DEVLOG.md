...
## 작업 내역
1. **DB 스키마 완벽 구축:** `users`, `products`, `orders`, `gifts` 4개 테이블 생성 및 FK 연동 완료. 이메일 UNIQUE 제약 및 1:1 매핑(order_id) 설정.
2. **시드 데이터 적재:** 기획 의도에 맞춰 '익산 환승패스' 테마 상품 18개와 bcrypt($2b$) 해시가 적용된 테스트 유저 3명 적재 완료.
3. **인프라 무중단 배포:** AWS EC2 내 MySQL 8 원격 접속 허용(0.0.0.0/0 임시 개방 후 IP 통제) 및 PM2를 활용한 백엔드 API 무중단 배포 성공.